...
//        float ax = CHASSIS.fdb.world.x_accel; 
        ...
        if (keyboard_data.Remote_KeyPush_B==1||(CHASSIS.rc->ch2<=-200)){  // 电脑键盘按下B键  删跳跃//(CHASSIS.rc->ch2<=-200)
            CHASSIS.step_time = 0;
...
static void Auto_ClimbStep(void)
{
    ...
    float swing_speed = 4.0f;   // 摆角速度
    float retract_speed = 0.8f; // 缩腿速度
    ...
    switch (climb_state) {
				...
				case 3:
        {
            ...
            if (is_phi_reached || climb_timer > 3000) {
                climb_state = 4; 
                climb_timer = 0;
            }
        } break;
    ...
    for (uint8_t i = 0; i < 2; i++) {
        ...
        float kp_phi = 40.0f, kd_phi = 1.0f;  
        ...
        CHASSIS.cmd.leg[i].joint.T[0] = fp32_constrain(CHASSIS.cmd.leg[i].joint.T[0], -30, 30);
        CHASSIS.cmd.leg[i].joint.T[1] = fp32_constrain(CHASSIS.cmd.leg[i].joint.T[1], -30, 30);
 ...
 if (CHASSIS.fdb.tell_gimbal_thing==1) 
{
     ...
     if (target_wz > current_wz) {
        ...
        // 计算平移方向dτ和正交方向dn的速度分量
		    
        float v_tau = s_x * sinf(CHASSIS.fdb.gimbal.gimbal_yaw_6020-GIMBAL_DIRECT_YAW_MID);  // 平移方向速度
        float v_n   =   s_y * cosf(CHASSIS.fdb.gimbal.gimbal_yaw_6020-GIMBAL_DIRECT_YAW_MID);    // 正交方向速度
 ...
 if (CHASSIS.fdb.tell_gimbal_thing==0) 
{
        ...
        if((CHASSIS.fdb.leg[0].rod.L0+CHASSIS.fdb.leg[1].rod.L0)/2>move_mid_length) //磕台阶速度
        {

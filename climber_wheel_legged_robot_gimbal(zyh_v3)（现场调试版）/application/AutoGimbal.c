...
#define HEARTBEAT_TIMEOUT 100  // 2????????
// 定义接收协议结构体
...
void AUTO_control_init(void)
{
	...
	// 初始化心跳检测变量
	visionDataStu.last_receive_time = HAL_GetTick();
	visionDataStu.aim_live = 0;  // 默认初始状态为未连接
}
...
// 心跳检测函数，需要在主循环中调用
void AutoGimbal_Heartbeat_Check(void)
{
    // 检查是否超时
    if((HAL_GetTick() - visionDataStu.last_receive_time) > HEARTBEAT_TIMEOUT)
    {
        visionDataStu.aim_live = 0;
    }
}

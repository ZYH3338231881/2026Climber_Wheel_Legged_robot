...
#define POWER_SAFETY_FACTOR   1.0f 
                              ...
                              float *I_mov_scale)
{
    ...
    // 1. 获取裁判系统限制 & 缓冲能量
    // ------------------------------------------------------------
     max_power = JudgementData.robot_status_t.chassis_power_limit;  
		if(SuperCap_rx.capEnergy>100)
		{
			max_power = JudgementData.robot_status_t.chassis_power_limit+30;  
		}
		else
		{
			max_power = JudgementData.robot_status_t.chassis_power_limit;  
    ...
    // 掉线保护：如果裁判系统没数据，默认限制 45w (步兵标准)
    if (max_power == 0) max_power = 45.0f;
		max_power=fp32_constrain(max_power,45,150);

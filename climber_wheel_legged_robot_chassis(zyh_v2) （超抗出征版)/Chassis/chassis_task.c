...
//        float ax = CHASSIS.fdb.world.x_accel; 
        ...
        if (!is_auto_climbing) 
        {
				if ((avg_theta > 0.22f && avg_theta_dot > 0.12f)&&((CHASSIS.rc->ch2>=100)||(keyboard_data.Remote_Key_F==1)||(CHASSIS.fdb.leg[0].rod.L0+CHASSIS.fdb.leg[1].rod.L0)>=0.7)) {
     ...
     if ((keyboard_data.Remote_KeyPush_B==1||(CHASSIS.rc->ch2<=-200)) && 
...
{
     target_wz =keyboard_data.Remote_Key_Ctrl * 11+(CHASSIS.rc->ch3==-660)*11+keyboard_data.Remote_Key_Shift*4;
 ...
 if (CHASSIS.fdb.tell_gimbal_thing==0) 
{
        ...
        { 
					...
					{
						target_v_set.vx += keyboard_data.Remote_Key_W*2.5*speed_direction;
            target_v_set.vx -= keyboard_data.Remote_Key_S*2.5*speed_direction;
					}
					else
					{
						target_v_set.vx += keyboard_data.Remote_Key_W*1.9*speed_direction;
            target_v_set.vx -= keyboard_data.Remote_Key_S*1.9*speed_direction;
					}
...
// 1. 动态获取当前允许的速度/旋转步长 (受功率严格制约)
    ...
    if (move_scale < 0.3f)   //功率吃紧
      ...
      if (SuperCap_rx.errorCode==0)
      {
            current_vx_step *= 0.7f; // 无超电  基本锁死目标值

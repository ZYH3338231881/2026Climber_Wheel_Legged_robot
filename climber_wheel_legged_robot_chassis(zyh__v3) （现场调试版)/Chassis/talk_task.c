...
extern void keyboard_sendControl2(uint8_t can, uint16_t std_id,uint8_t mouse_press_l,uint8_t mouse_press_r,fp32 yaw_speed,uint8_t fric_flag,uint8_t trigger_flag,fp32 shoot_heat_limit);
...
void Talk_Task_start(void const * argument)
{
  ...
  for(;;)
  {
		...
		if(CHASSIS.rc->ch2>=300&&CHASSIS.rc->ch2<=600)
		{
			TRIGGER_FLAG=1;
		}
		else if(CHASSIS.rc->ch2==660)
		{
			TRIGGER_FLAG=2;
		}
		else if(CHASSIS.rc->ch2==-660)
		...
		keyboard_sendControl2(3,0x557,keyboard_data.Remote_Mouse_KeyL,keyboard_data.Remote_Mouse_KeyR,CHASSIS.fdb.body.yaw_dot,FRIC_FLAG,TRIGGER_FLAG,JudgementData.robot_status_t.shooter_barrel_heat_limit);
		osDelay(1);
...
void keyboard_sendControl2(uint8_t can, uint16_t std_id,uint8_t mouse_press_l,uint8_t mouse_press_r,fp32 yaw_speed,uint8_t fric_flag,uint8_t trigger_flag,fp32 shoot_heat_limit)
{
    hcan_t * hcan = NULL;
    if (can == 1)
        hcan = &hfdcan1;
    ...
    int16_t yaw_speed_int16=(int16_t)((yaw_speed) * 1000);
		int16_t shoot_heat_limit_int16=(int16_t)((shoot_heat_limit));
    ...
    CAN_CTRL_DATA.tx_data[6] = shoot_heat_limit_int16>>8;
    CAN_CTRL_DATA.tx_data[7] = shoot_heat_limit_int16;
    canx_send_data(CAN_CTRL_DATA.hcan,CAN_CTRL_DATA.tx_header.Identifier,CAN_CTRL_DATA.tx_data,8);
}

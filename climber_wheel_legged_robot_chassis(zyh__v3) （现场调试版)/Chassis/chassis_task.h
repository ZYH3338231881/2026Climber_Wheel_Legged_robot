...
// 云台跟随用的单环  
#define KP_CHASSIS_FOLLOW_GIMBAL       (9) 
#define KI_CHASSIS_FOLLOW_GIMBAL       (0)
#define KD_CHASSIS_FOLLOW_GIMBAL       (30)
#define MAX_IOUT_CHASSIS_FOLLOW_GIMBAL (0)
#define MAX_OUT_CHASSIS_FOLLOW_GIMBAL  (8)
...
//ROLL轴跟踪角度环PID参数
#define KP_CHASSIS_PITCH_ANGLE        (2300)
#define KI_CHASSIS_PITCH_ANGLE        (0)
#define KD_CHASSIS_PITCH_ANGLE        (30000)   
#define MAX_IOUT_CHASSIS_PITCH_ANGLE  (0)
#define MAX_OUT_CHASSIS_PITCH_ANGLE   (180) 
 
// 腿长跟踪长度环PID参数
#define KP_CHASSIS_LEG_LENGTH_LENGTH        (2300.0f)
#define KI_CHASSIS_LEG_LENGTH_LENGTH        (0.0f)
#define KD_CHASSIS_LEG_LENGTH_LENGTH        (30000.0f)
#define MAX_IOUT_CHASSIS_LEG_LENGTH_LENGTH  (0.0f)
#define MAX_OUT_CHASSIS_LEG_LENGTH_LENGTH   (170.0f)   //----> 离地  120N
...
//追腿部PID 参数
//左腿追右腿
#define KP_CHASSIS_CAHSE_LEG_L_to_R        (300)
#define KI_CHASSIS_CAHSE_LEG_L_to_R        (0)
#define KD_CHASSIS_CAHSE_LEG_L_to_R        (500)
#define MAX_IOUT_CHASSIS_CAHSE_LEG_L_to_R  (0)
#define MAX_OUT_CHASSIS_CAHSE_LEG_L_to_R   (15)
//右腿追左腿
#define KP_CHASSIS_CAHSE_LEG_R_to_L        (300)
#define KI_CHASSIS_CAHSE_LEG_R_to_L        (0)
#define KD_CHASSIS_CAHSE_LEG_R_to_L        (500)
#define MAX_IOUT_CHASSIS_CAHSE_LEG_R_to_L  (0)
#define MAX_OUT_CHASSIS_CAHSE_LEG_R_to_L   (15)
...
#define J0_ANGLE_OFFSET     (-0.0880126953+M_PI) // (rad)关节0角度偏移量(电机0点到水平线的夹角)
#define J1_ANGLE_OFFSET     (0.0607852936)         // (rad)关节1角度偏移量(电机0点到水平线的夹角)
#define J2_ANGLE_OFFSET     (-11.0335159+ M_PI)  // (rad)关节2角度偏移量(电机0点到水平线的夹角)
#define J3_ANGLE_OFFSET     (-8.13079834)        // (rad)关节3角度偏移量(电机0点到水平线的夹角)
...
#define TAKE_OFF_FN_THRESHOLD (30.0f)  //待考量，建议可以用vofa打印出来
// 触地状态切换时间阈值，当时间接触或离地时间超过这个值时切换触地状态
#define TOUCH_TOGGLE_THRESHOLD (30)
#define MIN_LEG_LENGTH       ( 0.135f)         //最短腿长  
#define MAX_LEG_LENGTH       (0.357f)          //最大腿长  
...
typedef struct Body
{
    ...
    float phi;           //是车体相对于重力垂线的倾角（Pitch 俯仰角）
    float phi_dot;
...
typedef struct Leg
{
    struct rod
    {
        float Phi0;    // rad(Phi0)：是虚拟腿（从髋关节连到车轮中心的直线）相对于**车体（机座）**的夹角
        float dPhi0;   // rad/s
...
typedef struct __CTOM_message
{
  fp32 gimbal_yaw_6020;
	...
	uint8_t aim_live;  //1 自瞄开启  0  无自瞄模式
	uint8_t aim_shoot;  // 0不开火   2识别到开火

...
typedef struct
{
    ...
    uint16_t feedbackRefereeEnergyBuffer;//裁判系统的实时缓冲能量
    uint8_t resv1[3];
}SuperCap_tx_t;
/*-------------------- User functions --------------------*/
...
extern void CAN_cmd_supercap(hcan_t * hcan,uint16_t std_id,uint8_t status,uint16_t feedbackRefereePowerLimit, uint16_t feedbackRefereeEnergyBuffer);

...
void judge_painting_init(void)  
{
    ...
    make_cir(&pack_figs, 2, bg_aim_out, 4, 1, 2, 1, 5, 950, 490, 0, 360, 30, 0, 0); //紫红色准星
...
void judge_painting_update(void)
{
    ...
    if (send_cnt_aim > 0) {
        ...
        send_cnt_aim--;
            ...
            uint8_t aim_color = (ctom_message.aim_shoot == 2||ctom_message.aim_shoot==1) ? 2 : 4; // 2表示绿色，4表示紫色
            make_cir(&pack_figs, 0, bg_aim_out, aim_color, 2, 2, 1, 5, 950, 490, 0, 360, 30, 0, 0);

...
void _ui_update_frame1_Ungroup_0() {
    ...
    // 显示底盘运动速度
    ui_frame1_Ungroup_vx_float->number = CHASSIS.fdb.body.x_dot_obv*1000;
    
    //显示底盘旋转
    float yaw_error = CHASSIS.fdb.gimbal.gimbal_yaw_6020 - GIMBAL_DIRECT_YAW_MID;
    float center_x = (ui_frame1_Ungroup_chassis->start_x + ui_frame1_Ungroup_chassis->end_x) / 2.0f;
    float center_y = (ui_frame1_Ungroup_chassis->start_y + ui_frame1_Ungroup_chassis->end_y) / 2.0f;
    float width = ui_frame1_Ungroup_chassis->end_x - ui_frame1_Ungroup_chassis->start_x;
    float height = ui_frame1_Ungroup_chassis->end_y - ui_frame1_Ungroup_chassis->start_y;
    ui_frame1_Ungroup_chassis->start_x = center_x - width * 0.5f * cos(yaw_error) + height * 0.5f * sin(yaw_error);
    ui_frame1_Ungroup_chassis->start_y = center_y - width * 0.5f * sin(yaw_error) - height * 0.5f * cos(yaw_error);
    ui_frame1_Ungroup_chassis->end_x = center_x + width * 0.5f * cos(yaw_error) + height * 0.5f * sin(yaw_error);
    ui_frame1_Ungroup_chassis->end_y = center_y + width * 0.5f * sin(yaw_error) - height * 0.5f * cos(yaw_error);
...
void _ui_update_frame1_Ungroup_1() {
    for (int i = 0; i < 7; i++) {
		...
		ui_frame1_Ungroup_gyro_float->number=CHASSIS.fdb.body.yaw_dot*1000;//显示底盘角速度
